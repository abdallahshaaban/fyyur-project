...
# app.config['SQLALCHEMY_DATABASE_URI'] = 'postgresql://postgres:admin@localhost:5432/artist_venues_database'
...
def search_venues():
  ...
  for obj in search_result:
    result.append({
      "id":obj.id,
      "name":obj.name,
      "num_upcoming_shows":len(db.session.query(Show).filter(Show.venue_id == obj.id).filter(Show.start_time > datetime.now()).all()),
    })
  response={
...
def delete_venue(venue_id):
  ...
  if not error: 
    flash('Venue ' + venue_id + 'was not successfully deleted')
  return render_template('pages/home.html')
#  Artists
#  ----------------------------------------------------------------
@app.route('/artists')
...
def search_artists():
  ...
  search_term=request.form.get('search_term', '')
  # searchInput = request.form["search_term"]
  ...
  for obj in search_result:
    result.append({
      "id":obj.id,
      "name":obj.name,
      "num_upcoming_shows":len(db.session.query(Show).filter(Show.artist_id == obj.id).filter(Show.start_time > datetime.now()).all()),
    })
  response={
    "count": len(search_result),
    "data": result
...
def create_show_submission():
  ...
  try:
    ...
    venuetId = request.form['venue_id']
    start_time = request.form['start_time']

    show = Show(artist_id=artistId,venue_id=venuetId,start_time=start_time)
